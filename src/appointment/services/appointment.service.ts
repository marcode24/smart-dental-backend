...
export class AppointmentService {
  constructor(
    ...
    private appointmentDetailModel: typeof AppointmentDetail,
  ) {}
  ...
  async create(data: CreateAppointmentDto) {
    ...
    id_record.forEach(async (id_record) => {
      await this.appointmentDetailModel.create({
        id_appointment,
        id_record: id_record,
      });
    });
  ...
  async findByUser(
    ...
    optionsParams: ISearchParams,
  ) {
    status = status || StatusAppointment.PENDING;
    let { limit, offset } = optionsParams;
    const { fullname, date } = optionsParams;
    ...
    if (fullname) {
      ...
      return {
        appointments: await this.appointmentModel.findAll(optionsQuery),
      };
    }
    if (date) {
      optionsQuery = {
  ...
  async changeStatus(id_appointment: number, data: ChangeStatusAppointmentDto) {
    const appointmentFound = await this.appointmentModel.findByPk(
      id_appointment,
    );
    if (!appointmentFound) {
      return new NotFoundException(
        `appointment not found with id ${id_appointment}`,
      );
    }
