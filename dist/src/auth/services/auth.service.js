...
let AuthService = class AuthService {
    ...
    async validateUser(username, password) {
        ...
        if (user.password) {
            ...
            if (isMatch) {
                const { password } = user, response = __rest(user, ["password"]);
                return response;
            }
        }
    ...
    async renewToken(userId) {
        ...
        if (!userFound.status) {
            return new common_1.ForbiddenException('User disabled');
        }
        return this.generateJWT(userFound);
    }
    async generateJWT(user) {
        const { role, id_user } = user;
        const payload = { role, sub: id_user };
        return {
