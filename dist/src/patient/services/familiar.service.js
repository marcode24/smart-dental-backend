"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FamiliarService = void 0;
const common_1 = require("@nestjs/common");
const sequelize_1 = require("@nestjs/sequelize");
const familiar_entity_1 = require("../entities/familiar.entity");
let FamiliarService = class FamiliarService {
    constructor(familiarModel) {
        this.familiarModel = familiarModel;
    }
    create(data) {
        return this.familiarModel.create(Object.assign({}, data));
    }
    update(familiarID, changes) {
        return this.familiarModel.update(changes, { where: { id_familiar: familiarID } });
    }
};
FamiliarService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, sequelize_1.InjectModel)(familiar_entity_1.Familiar)),
    __metadata("design:paramtypes", [Object])
], FamiliarService);
exports.FamiliarService = FamiliarService;
//# sourceMappingURL=familiar.service.js.map